...
try:
    ...
    fedapay = None # La vérification est toujours utile, mais les appels seront remplacés
...
def get_payment_session():
    ...
    # Configuration de la stratégie de relance (Retry)
...
# =====================================================================
# 🛠️ NOUVELLE FONCTION UTILITAIRE : Interrogation FedaPay par HTTP
# =====================================================================
def get_fedapay_transaction_status(fedapay_id: str) -> str:
    ...
    try:
        ...
        raise # On propage l'erreur pour la gérer dans CheckTransactionStatusView


# =====================================================================
# 🔵 1️⃣ CREATION D’UNE TRANSACTION (CORRIGÉE : return_url)
# =====================================================================
class CreateTransactionView(APIView):
    """Crée une transaction via l'API REST FedaPay avec callback_url personnalisé."""
    ...
    def post(self, request):
        ...
        # 2. Créer IMMÉDIATEMENT la transaction en DB pour avoir l'ID local
        ...
        local_transaction_id = transaction_obj.id  # ✅ ID local Django
        
        # 3. Préparation de la Requête FedaPay avec l'ID local dans callback_url
        session = get_payment_session()
        fedapay_url = f"{settings.FEDAPAY_API_URL}/transactions"
        
        # ✅ SOLUTION: Inclure l'ID local dans le callback_url
        ...
        payload = {
            ...
            "callback_url": callback_url,  # ✅ URL avec ID local
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                if not all([fedapay_transaction_id, payment_url]):
                    logger.error(f"❌ Données critiques FedaPay manquantes")
                    # Supprimer la transaction temporaire
                    transaction_obj.delete()
                    raise Exception("La réponse FedaPay est incomplète.")
                
                # 6. Mettre à jour avec les vraies données FedaPay
                with db_transaction.atomic():
                ...
                logger.info(f"✅ Transaction créée: ID local={transaction_obj.id}, FedaPay={fedapay_transaction_id}")
...
# =====================================================================
# 🟢 2️⃣ CALLBACK FedaPay (Redirection vers le Front-end) (INCHANGÉ)
# =====================================================================
@api_view(['GET'])
...
# =====================================================================
# 🟡 3️⃣ VÉRIFICATION DU STATUT POST-REDIRECTION (CORRIGÉE : Utilisation de requests)
# =====================================================================
class CheckTransactionStatusView(APIView):
    ...
    Renvoie download_credit_id si approuvé pour téléchargement unique.
    """
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request, transaction_id): # transaction_id est l'ID local Django
        ...
        # 2. Vérifier le statut actuel en DB
        current_status = transaction.status
        
        if current_status != 'approved':
            # 3. Si non approuvée, vérifier auprès de FedaPay via requêtes HTTP
            try:
                ...
                if fedapay_status == 'approved':
                    ...
                    current_status = 'approved' # Mettre à jour le statut après activation
                    ...
                    with db_transaction.atomic():
                        ...
                        transaction.save()
                    current_status = fedapay_status
            
            except Exception as e:
                logger.error(f"❌ Erreur vérification FedaPay pour {fedapay_id}: {str(e)}", exc_info=True)
                # Si erreur externe, retourner un statut temporaire/erreur
                return Response(
                    {'error': 'Erreur serveur lors de la vérification externe', 'status': current_status},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )

        # 4. Préparer et retourner la réponse finale
        response_data = {
        ...
        # AJOUT CRITIQUE: Ajouter l'ID du crédit si la transaction est approuvée pour téléchargement unique
        if current_status == 'approved' and transaction.transaction_type == 'one_time_download' and transaction.cv:
            try:
                # On recherche le crédit qui doit exister
                credit = DownloadCredit.objects.get(
                    user=user, 
                    cv=transaction.cv, 
                    transaction=transaction, 
                    is_used=False # Uniquement le crédit non consommé
                )
                response_data['download_credit_id'] = credit.id # ✅ ID transmis
            except DownloadCredit.DoesNotExist:
                logger.error(f"❌ Crédit non trouvé après approbation pour T_ID:{transaction.id}. Vérifiez _handle_approved_transaction.")

        return Response(response_data)
            
    # La méthode _handle_approved_transaction n'a pas besoin d'être modifiée.
    @db_transaction.atomic
    def _handle_approved_transaction(self, transaction_obj):
        # ... (Logique inchangée pour créer le Premium ou le DownloadCredit)
        ...
        # CAS 2: TÉLÉCHARGEMENT UNIQUE
            ...
            # Mise à jour du compteur utilisateur (le crédit réel est l'objet DownloadCredit)
            user.download_credits = (user.download_credits or 0) + 1
            user.save()


# =====================================================================
# 🔴 4️⃣ WEBHOOK FedaPay — APPROBATION & ÉCHEC (INCHANGÉ)
# =====================================================================
class FedaPayWebhookView(APIView):
    # ... (Le code du Webhook reste inchangé)
    permission_classes = [AllowAny]
